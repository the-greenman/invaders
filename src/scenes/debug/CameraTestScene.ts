...
export class CameraTestScene extends Phaser.Scene {
  ...
  private faceKey = 'detected-face';
  private statusText!: Phaser.GameObjects.Text;
  private liveImage?: Phaser.GameObjects.Image;
  private faceImage?: Phaser.GameObjects.Image;
  private lastBBox: { xMin: number; yMin: number; width: number; height: number } | null = null;
  ...
  private handleDetection(results: any) {
    ...
    if (!box) return;
    // Mediapipe gives normalized values; convert to pixel space for drawing
    const w = this.videoEl.videoWidth || 640;
    const h = this.videoEl.videoHeight || 480;
    const xMin = (box.xMin ?? box.xCenter - box.width / 2) * w;
    const yMin = (box.yMin ?? box.yCenter - box.height / 2) * h;
    const width = (box.width ?? box.w ?? 0.25) * w;
    const height = (box.height ?? box.h ?? 0.25) * h;
  ...
  private async showDetectedFace() {
    if (!this.videoEl) return;
    try {
      const faceData = await FaceManager.captureAndSaveFace(this.videoEl, this.lastBBox ?? undefined);
      ...
      image.onload = () => {
        ...
        this.textures.addImage(this.faceKey, image);
  ...
  update(): void {
    ...
    if (this.lastBBox) {
      ...
      ctx.lineWidth = 3;
      ctx.strokeRect(this.lastBBox.xMin, this.lastBBox.yMin, this.lastBBox.width, this.lastBBox.height);
    }
    tex.refresh();
  }
}
